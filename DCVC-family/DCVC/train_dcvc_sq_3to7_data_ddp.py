            ...
            for frame_pos in range(seq_length):
                if frame_pos == 0:  # First frame (I-frame) in each sequence
                    ...
                    if frame_pos == seq_length - 1: 
                        # Skip due to lack of next frame
                        continue
                    ...
                    # Process all P-frames in the batch with their corresponding reference frames
                    # DCVC model expects reference and current frames in the same batch size
                    IP_result = model(reference_frames, current_frames, training=True, stage=stage)

                    # Calculate loss (already accounts for batch size, just normalize by accumulation steps)
                    IP_loss = IP_result["loss"]

                    PP_result = model(IP_result["pixel_rec"] if stage==1 else IP_result["recon_image"], next_frames, training=True, stage=stage)

                    # Calculate loss (already accounts for batch size, just normalize by accumulation steps)
                    PP_loss = PP_result["loss"]

                    # Combine losses
                    loss = (IP_loss + PP_loss)/2
                    ...
                    # Collect statistics
                    batch_loss += (IP_result["loss"].item() + PP_result["loss"].item())/2
                    total_mse += (IP_result["mse_loss"].item() * batch_size + PP_result["mse_loss"].item() * batch_size)/2  # Account for all frames in batch
                    total_bpp += (IP_result["bpp_train"].item() * batch_size + PP_result["bpp_train"].item() * batch_size)/2
                    total_psnr += (-10 * math.log10(IP_result["mse_loss"].item()) * batch_size + -10 * math.log10(PP_result["mse_loss"].item()) * batch_size)/2
                    n_frames += batch_size  # Count all frames in batch
            ...
            if finetune and stage == 4: 
                ...
                for frame_pos in range(seq_length):
                    if frame_pos == 0:
                        ...
                        total_mse += (IP_result["mse_loss"].item() * batch_size + PP_result["mse_loss"].item() * batch_size)/2
                        total_bpp += (IP_result["bpp_train"].item() * batch_size + PP_result["bpp_train"].item() * batch_size)/2
                        total_psnr += (-10 * math.log10(IP_result["mse_loss"].item()) * batch_size + -10 * math.log10(PP_result["mse_loss"].item()) * batch_size)/2
                        n_frames += batch_size
...
def main():
    ...
    parser.add_argument('--checkpoint_dir', type=str, default='results/checkpoints_data_ddp_3f', help='Directory to save checkpoints')
    parser.add_argument('--log_dir', type=str, default='results/logs_data_ddp_3f', help='Directory to save logs')
