        ...
        if finetune and stage == 4:
            ...
            for frame_pos in range(seq_length):
                ...
                if frame_pos == 0:  # First frame (I-frame) in each sequence
                    ...
                    if frame_pos ==seq_length-1:  # Last frame (P-frame) in each sequence
                        # skip last frame processing
                        continue
                    ...
                    # Collect statistics
                    batch_loss += (IP_loss + PP_loss).item() / 2
                    total_mse += (IP_result["mse_loss"].item() + PP_result["mse_loss"].item()) / 2 * batch_size  # Account for all frames in batch
...
def main():
    ...
    parser.add_argument('--checkpoint_dir', type=str, default='results/checkpoints_data_3f', help='Directory to save checkpoints')
    parser.add_argument('--log_dir', type=str, default='results/logs_data_3f', help='Directory to save logs')
